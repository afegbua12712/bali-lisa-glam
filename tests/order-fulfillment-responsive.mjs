...
import { read, fixture, views } from './order-view-fixtures.mjs'
const output=resolve('node_modules/.tmp/order-fulfillment-browser')
...
const server=createServer((req,res)=>{
  ...
  const order={...fixture,status,payment_status:status==='pending'?'awaiting_payment':'paid',inventory_reservation_status:status==='pending'?'reserved':'committed',shipped_at:status==='shipped'||status==='delivered'?'2026-09-21T10:00:00Z':null,delivered_at:status==='delivered'?'2026-09-22T10:00:00Z':null}
  const html=renderToStaticMarkup(createElement(view==='admin'?AdminOrders:CustomerOrders,{orders:[order],refresh:async()=>{},refreshEmails:async()=>{},note:()=>{}}))
...
try {
  ...
  const results=[]
  for(const width of [1280,1024,768,390,320]) for(const theme of ['light','dark','default']) for(const view of ['customer','admin']) for(const status of ['pending','processing','shipped','delivered']) {
    ...
    const measured=await call('Runtime.evaluate',{expression:`JSON.stringify({
      ...
      overflow:[...document.querySelectorAll('.order-progress li,.order-card-header,.order-actions button,.order-open,.fulfillment-action button')].filter(e=>{const r=e.getBoundingClientRect();return r.left < -1 || r.right > innerWidth+1 || e.scrollWidth > e.clientWidth+2}).map(e=>e.className+':'+e.textContent.slice(0,60)),
  ...
  const failures=results.filter(r=>r.scroll>r.width+1||r.overflow.length||!r.reference||r.theme!==(r.preference==='default'?'dark':r.preference)||r.progress!==({pending:1,processing:3,shipped:4,delivered:5})[r.status])
