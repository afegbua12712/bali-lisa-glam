-- Bali Lisa Glam production commerce schema. Run with Supabase CLI or SQL Editor.
...
begin
  ...
  if jsonb_array_length(lines) = 0 then raise exception 'Order needs at least one item'; end if;
  for line in select * from jsonb_array_elements(lines) loop
    ...
    if product_row.inventory_quantity < (line ->> 'quantity')::integer then raise exception 'Insufficient inventory for %', product_row.name; end if;
  ...
  values (auth.uid(), subtotal, shipping, subtotal + shipping, shipping_address) returning id into new_order_id;
  for line in select * from jsonb_array_elements(lines) loop
...
-- Seed catalog for the initial launch. Images are public CDN URLs; move them to Storage when brand assets are ready.
insert into public.categories(name, slug, sort_order) values ('Complexion','complexion',1),('Lips','lips',2),('Eyes','eyes',3),('Cheeks','cheeks',4),('Skincare','skincare',5);
...
from (values
('complexion','Luminous Veil Foundation','luminous-veil-foundation','A skin-loving, breathable foundation with a radiant satin finish that looks like you, only more luminous.',3800,'https://images.unsplash.com/photo-1631214524020-7e18db9a8f92?auto=format&fit=crop&w=900&q=85','["Porcelain","Warm Beige","Honey","Espresso"]',4.9,126,120,'Bestseller',false),
('lips','Velvet Kiss Lip Color','velvet-kiss-lip-color','Weightless, velvet-matte colour that stays soft and comfortable through every toast and selfie.',2400,'https://images.unsplash.com/photo-1586495777744-4413f21062fa?auto=format&fit=crop&w=900&q=85','["Rosewood","Terracotta","Berry","Sienna"]',4.8,88,80,null,true),
('skincare','Golden Hour Glow Oil','golden-hour-glow-oil','A silky botanical face oil that cushions skin in moisture and a soft, sunlit glow.',3200,'https://images.unsplash.com/photo-1608248597279-f99d160bfcbc?auto=format&fit=crop&w=900&q=85','["Universal"]',5.0,64,65,null,false),
('eyes','Cloud Lash Mascara','cloud-lash-mascara','Buildable volume with a featherlight feel. Lifts, defines, and never weighs lashes down.',2600,'https://images.unsplash.com/photo-1599305090598-fe179d5012278-27bfdc403348?auto=format&fit=crop&w=900&q=85','["Onyx"]',4.7,107,85,null,false)
) as v(category_slug,name,slug,description,price_cents,image_url,shades,rating,review_count,inventory_quantity,badge,is_new) join public.categories c on c.slug=v.category_slug;
