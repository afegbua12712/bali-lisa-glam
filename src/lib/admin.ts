...
export async function advanceOrderFulfillment(id: string, expectedStatus: string, nextStatus: 'processing' | 'shipped' | 'delivered', carrier = '', tracking = '') {
  const { error } = await supabase.rpc('record_order_shipment', { target_order_id: id, expected_status: expectedStatus, next_status: nextStatus, carrier, tracking })
  if (error) throw error
}
