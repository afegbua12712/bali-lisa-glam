...
export async function getCustomerAccount() {
  ...
  const [{ data: profile, error: profileError }, { data: orders, error: orderError }, { data: address, error: addressError }] = await Promise.all([
    ...
    supabase.from('orders').select('id,order_reference,status,shipping_method,shipment_carrier,tracking_number,processing_at,shipped_at,delivered_at,payment_method,payment_status,paid_at,payment_expires_at,currency,total_cents,subtotal_cents,shipping_cents,shipping_address,created_at,order_items(product_id,product_name,shade,selected_options,quantity,unit_price_cents)').eq('customer_id', user.id).order('created_at', { ascending: false }),
